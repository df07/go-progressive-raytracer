...
func TestSampleLightEmission(t *testing.T) {
	// Test with no lights
	emptyLights := []Light{}
	emptyLightSampler := NewUniformLightSampler(emptyLights, 10.0)
	_, found := SampleLightEmission(emptyLights, emptyLightSampler, NewRandomSampler(rand.New(rand.NewSource(42))))
	if found {
	...
	singleLights := []Light{mockLight}
	singleLightSampler := NewUniformLightSampler(singleLights, 10.0)

	random := rand.New(rand.NewSource(42))
	sample, found := SampleLightEmission(singleLights, singleLightSampler, NewRandomSampler(random))
	...
	multiLightSampler := NewUniformLightSampler(multiLights, 10.0)

	sample2, found2 := SampleLightEmission(multiLights, multiLightSampler, NewRandomSampler(random))
	if !found2 {
