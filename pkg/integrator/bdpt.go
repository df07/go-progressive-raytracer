...
// AlternativeStrategyPath represents the PDF values for an alternative BDPT strategy
type AlternativeStrategyPath struct {
	ForwardPDFs      []float64
	ReversePDFs      []float64
	IsConnectible    []bool // true if this vertex can be connected to (not specular, not delta light)
	Length           int
	CameraPathLength int // t: number of vertices in camera path
}

// calculateMISWeightAlt2 implements MIS weighting by separating path building from RI calculation
// This approach builds the alternative strategy path first, then calculates sumRi in a clean loop
func (bdpt *BDPTIntegrator) calculateMISWeightAlt2(cameraPath, lightPath Path, sampledVertex *Vertex, s, t int, scene core.Scene) float64 {
...
// buildAlternativeStrategyPath constructs the PDF arrays needed for MIS weight calculation
// This encapsulates all the complex PDF remapping logic in one place
func (bdpt *BDPTIntegrator) buildAlternativeStrategyPath(cameraPath, lightPath *Path, sampledVertex *Vertex, s, t int, scene core.Scene) AlternativeStrategyPath {
	totalLength := s + t
	altPath := AlternativeStrategyPath{
		ForwardPDFs:      make([]float64, totalLength),
		ReversePDFs:      make([]float64, totalLength),
		IsConnectible:    make([]bool, totalLength),
		Length:           totalLength,
		CameraPathLength: t,
	}

	// Copy camera path vertices (indices 0 to t-1)
	for i := 0; i < t; i++ {
		vertex := &cameraPath.Vertices[i]
		altPath.ForwardPDFs[i] = vertex.AreaPdfForward
		altPath.ReversePDFs[i] = vertex.AreaPdfReverse
		altPath.IsConnectible[i] = !vertex.IsSpecular // Camera vertices: connectible if not specular
	}

	// Copy light path vertices (indices t to t+s-1)
	for i := 0; i < s; i++ {
		vertex := &lightPath.Vertices[i]
		altPath.ForwardPDFs[t+i] = vertex.AreaPdfForward
		altPath.ReversePDFs[t+i] = vertex.AreaPdfReverse
